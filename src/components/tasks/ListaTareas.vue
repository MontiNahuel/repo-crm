...
<template>
    ...
    <ul v-else class="divide-y divide-border-main transition-colors">
        <li v-for="tarea in tareas" :key="tarea.id" 
            ...
            <button @click="$emit('toggle', tarea)" 
            ...
            <div class="flex-1 min-w-0 cursor-pointer" @click="$emit('toggle', tarea)">
            ...
            <div class="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 shrink-0">
                <button @click="$emit('editar', tarea)" class="p-2 text-text-muted hover:text-blue-500 transition-colors rounded-lg hover:bg-bg-main" title="Editar">
                    ✏️
                </button>
                <button @click="$emit('eliminar', tarea.id)" class="p-2 text-text-muted hover:text-red-500 transition-colors rounded-lg hover:bg-bg-main" title="Eliminar">
                    🗑️
                </button>
            </div>
        </li>
