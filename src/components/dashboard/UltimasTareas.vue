...
<template>
    <div class="bg-sidebar p-6 rounded-2xl shadow-sm border border-border-main flex flex-col h-full transition-colors duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <Teleport to="body">
                <CrearTareaModal v-if="mostrarModal" @close="mostrarModal = false" @tarea-creada="recargarTareas"/>
            </Teleport>
        </div>
        ...
        <ul v-else class="space-y-3 flex-1 overflow-y-auto">
            <li v-for="tarea in tareas" :key="tarea.id" 
                ...
                <Teleport to="body">
                    <ModalConfirmacion v-if="mostrarModalEliminar" titulo="Eliminar tarea" mensaje="¿Estás seguro de que querés eliminar esta tarea del To-Do? Esta acción no se puede deshacer." textoConfirmar="Sí, eliminar" :cargando="eliminando" @confirmar="confirmarEliminacion" @cancelar="cancelarEliminacion" />
                </Teleport>
                <Teleport to="body">
                    <CrearTareaModal v-if="mostrarModal" :tareaAEditar="tareaSeleccionada" @close="mostrarModal = false" @tarea-creada="guardarTareaEditada" />
                </Teleport>
